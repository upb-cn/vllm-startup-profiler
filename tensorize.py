...
def main():
    ...
    if args.command == "serialize":
        ...
        output_dir = tensorizer_dir.rstrip("/")

        # Ensure directory exists
        os.makedirs(output_dir, exist_ok=True)

        # Choose filename based on tensor parallelism
        if engine_args.tensor_parallel_size > 1:
            model_path = f"{output_dir}/model-rank-%03d.tensors"
        else:
            model_path = f"{output_dir}/model.tensors"
        ...
        if args.lora_path:
            tensorizer_config.lora_dir = output_dir
